...
if st.session_state.stage == "début":
    ...
    liste_reponse["recommandation"] = st.text_input("Il m'a été recommandé par :", max_chars=25, placeholder="Nom de la personne", key="recommandation", value=liste_reponse.get("recommandation", ""))
    ...
    liste_reponse["alimentation_sante"] = st.select_slider(
        ...
        ["Pas du tout", "Plutôt non", "Ca dépends", "Plutôt oui", "Tout à fait"],
        value=liste_reponse.get("alimentation_sante", "Ca dépends"),
    ...
    liste_reponse["importance_alimentation"] = st.slider(
        ...
        0, 10, liste_reponse.get("importance_alimentation", 5),
    ...
    liste_reponse["satisfaction_alimentation"] = st.slider(
        ...
        0, 10, liste_reponse.get("satisfaction_alimentation", 5),
    ...
    liste_reponse["etat_bien_etre"] = st.select_slider(
        ...
        value=liste_reponse.get("etat_bien_etre", "Moyen"),
        key="etat_bien_etre"
    )
...
if st.session_state.stage == "deuxième bloc":
    liste_reponse2 = {}
    # Préremplir les champs avec les réponses précédentes si elles existent
    if 'liste_reponse2' in st.session_state:
        liste_reponse2 = st.session_state.liste_reponse2
    
    if liste_reponse2 not in st.session_state:
        st.session_state.liste_reponse2 = {}
        st.session_state.liste_reponse2["choix_multiple"] = []
    ...
    liste_reponse2["theme_prefere"] = st.text_input("Parmi les thèmes abordés précédemment, quel est celui sur lequel vous seriez le plus intéressé(e) à en savoir plus ?", key="theme_prefere", value=liste_reponse2.get("theme_prefere", ""))

    boolean_liste = ["non","oui"]
    liste_reponse2["accompagnement_perso"] = st.radio("Si vous deviez changer quelque chose dans votre quotidien, pensez-vous qu'un accompagnement personnalisé et gratuit soit un plus ?",boolean_liste , key="accompagnement_perso", index= boolean_liste.index(liste_reponse2.get("accompagnement_perso","non")))
    liste_reponse2["interlocuteur_perso"] = st.radio("Lorsque vous commandez en ligne, préférez-vous avoir un interlocuteur identifié qui puisse vous accompagner au besoin ?", boolean_liste, key="interlocuteur_perso", index= boolean_liste.index(liste_reponse2.get("interlocuteur_perso","non")))
...
if st.session_state.stage == "deuxième bloc":
    col1_deuxieme, col2_deuxieme = st.columns(2)
    with col1_deuxieme:
        st.button("Retour", on_click=set_stage, args=["début"], key="deuxième_début")
    with col2_deuxieme:
...
if st.session_state.stage == "troisième bloc":
    ...
    # Préremplir les champs avec les réponses précédentes si elles existent
    if 'liste_reponse3' in st.session_state:
        liste_reponse3 = st.session_state.liste_reponse3
    ...
    liste_reponse3["rendez_vous"] = st.radio(" ", liste_choix_rendezvous, key="rendez_vous", index = liste_choix_rendezvous.index(liste_reponse3.get("rendez_vous","Je souhaite prendre rendez-vous pour une présentation personnalisée")))
    
    if liste_reponse3["rendez_vous"] == "Je souhaite être rappelé à partir du  :":
        liste_reponse3["date"] = st.date_input("", key="date", value=liste_reponse3.get("date",None))
...
if st.session_state.stage == "troisième bloc":
    col1_deuxieme, col2_deuxieme = st.columns(2)
    with col1_deuxieme:
        st.button("Retour", on_click=set_stage, args=["deuxième bloc"], key="troisième-deuxième")
    with col2_deuxieme:
...
if st.session_state.stage == "quatrième bloc":
    # Intro
    "Pour que nous puissions vous recontacter, merci de remplir les informations ci-dessous :"
    
    # Préremplir les champs avec les réponses précédentes si elles existent
    liste_reponse4 = {}
    if 'liste_reponse4' in st.session_state:
        liste_reponse4 = st.session_state.liste_reponse4
    
    # questionnaire de contacts
    
    liste_reponse4["prenom"] = st.text_input("Prénom", key="prenom", autocomplete="given-name", max_chars=25, value=liste_reponse4.get("prenom",""))
    liste_reponse4["nom"] = st.text_input("Nom", key="nom", autocomplete="family-name", max_chars=25, value=liste_reponse4.get("nom",""))
    liste_reponse4["adresse"] = st.text_input("Adresse", key="adresse", autocomplete="adress", max_chars=150, value=liste_reponse4.get("adresse",""))
    liste_reponse4["mail"] = st.text_input("Mail", key="mail", autocomplete="email", max_chars=50, value=liste_reponse4.get("mail",""))
    ...
    liste_reponse4["telephone"] = st.text_input("Téléphone", key="telephone", autocomplete="tel", max_chars=15, value=liste_reponse4.get("telephone",""))
    ...
    liste_reponse4["horaire_appel"] = st.text_input("Jours et horaires d'appels", key="horaire_appel", value=liste_reponse4.get("horaire_appel",""))
...
if st.session_state.stage == "quatrième bloc":
    ...
    col1_deuxieme, col2_deuxieme = st.columns(2)
    with col1_deuxieme:
        st.button("Retour", on_click=set_stage, args=["troisième bloc"], key="quatrieme-troisieme")
    with col2_deuxieme:
